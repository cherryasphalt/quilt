...
class InviteCode(
    ...
    companion object {
        ...
        fun fromString(from: String): InviteCode? {
            ...
            val domainRegex = Regex("^((\\*)|((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)|((\\*\\.)?([a-zA-Z0-9-]+\\.){0,5}[a-zA-Z0-9-][a-zA-Z0-9-]+\\.[a-zA-Z]*))(?=:)")
            val domain = domainRegex.find(from)?.value
            ...
            return InviteCode(domain!!, port!!, pubKey!!, inviteKey!!)
        }
    }

    fun getInviteCode(): String {
        return "$domain:$port:${pubKey.getIdentifierString()}~$inviteKey"
    }
}